...
class Individual:
    def __init__(self, p=[None]):
        self.p = p
        self.g = [self.p[_] - RANGES[_][0] for _ in range(D)]
    ...
    def __and__(self, Ind2):  # pylint: disable=C0103
        ...
        if (np.array(chi1.g) >= 0).all() and (
            np.array([chi1.p[i] - RANGES[i][1] for i in range(D)]) <= 0
        ).all():
            chi1.p = [chi1.g[i] + RANGES[i][0] for i in range(D)]
        else:
            chi1.survive = False

        if (np.array(chi2.g) >= 0).all() and (
            np.array([chi2.p[i] - RANGES[i][1] for i in range(D)]) <= 0
        ).all():
    ...
    def __repr__(self):
        ...
        for i in range(D):
            s += (
                "0" * (GENELENS[i] - len(bin(self.g[i])[2:])) + bin(self.g[i])[2:] + " "
            )
        print(s, "\n")
    ...
    def calFit(self):
        ...
        os.makedirs(path)
        geo.update(self.p, geodir=path + os.sep)
